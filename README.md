...
26개 글로벌 자산(미국·한국·신흥·선진국 주식, 크립토, 채권, 원자재, 부동산, FX, VIX)을 `yfinance`로 수집하고, 모멘텀·변동성·상관관계·시장 레짐을 분석해 6가지 포트폴리오 전략을 **look-ahead bias 없이** 백테스팅합니다. 단순히 수익률을 비교하는 데서 그치지 않고, **out-of-sample 검증**과 **다중검정 보정(Deflated Sharpe)** 으로 *"이 성과가 과적합인가, 진짜인가"* 까지 답하는 것이 목표입니다.

> **교육·연구 목적**입니다. 실제 투자 판단에 사용하지 마세요. 과거 성과는 미래를 보장하지 않습니다.
...
- **설정 중심** — 자산·전략·수수료·기간을 YAML로 관리, 코드 수정 없이 실험
- **선택적 리스크 오버레이** — 포지션 캡 + 상관 급등 디레버리징
...
## 대표 전략: Top-N Momentum

가장 견고한 위험조정·강건성을 보인 플래그십 전략입니다. 비슷한 수익을 더 낮은 낙폭·회전율로 달성합니다.

### 작동 방식

매월 말, 22개 거래가능 자산 각각에 **12개월 총수익률**로 모멘텀 점수를 매기고:

1. 점수 **상위 5개** 선발
2. 그중 자기 모멘텀이 음수인 자산은 제외하고 그 자리는 **현금** (절대모멘텀 필터 = 약세장 방어)
3. 살아남은 자산에 **동일비중**(1/N)
4. 다음 달 재평가 — 순위가 바뀌면 갈아탐

한 줄로: *"전 자산군에서 요즘 가장 견고하게 오르는 5개에 올라타되, 다 같이 하락하면 현금으로 피한다."* 돈이 주식 → 원자재 → 채권 → 신흥국으로 옮겨갈 때 **항상 주도 자산에 올라타는** 구조입니다.

### 어떻게 튜닝했나 (`scripts/topn_sweep.py`)

4개 노브를 격자 탐색(48개 조합)하고, 각 조합을 **전체기간 / 검증창 / 테스트창**에서 평가해 *out-of-sample 일치 + Deflated Sharpe* 로 강건성을 판정했습니다:

- **룩백**: 12-6-3 블렌드 · 단일 12개월 · 12-1(최근 1개월 제외) · 6개월 · 3개월 · 12-6
- **보유 종목 수 N**: 3 / 5 / 7 / 10
- **현금 필터**: on / off

### 결과 — 단순함이 이긴다

- **단일 12개월 모멘텀**이 블렌드를 압도. OOS 테스트 Sharpe **0.94 → 1.11**(낙폭은 동일). 3·6개월 성분은 노이즈만 더했고, 12개월이 가장 끈질긴 신호였습니다.
- **3개월 모멘텀은 과적합**. 전체기간만 좋아 보였을 뿐 검증창 Sharpe가 0.50으로 붕괴 — 빠른 모멘텀은 불안정.
- **12-1 skip(학술 표준)은 여기선 손해**. 다자산 universe에선 최근 1개월에도 유효 모멘텀이 있었습니다.
- **N=5가 스위트스폿**. N=3는 약간 높은 수익이지만 −60% 낙폭, N=10은 과분산으로 열화.
- **Deflated Sharpe = 1.00**. 48개를 탐색했어도 모멘텀 엣지는 다중검정을 가뿐히 통과 — 데이터 스누핑이 아닙니다.

채택한 설정: **12개월 / N=5 / 필터 on** (`config/strategy.yaml`의 `strategies.topn`). 전체기간 Sharpe 0.73→**0.76**, Calmar 0.44→**0.45**, 테스트창 Sharpe 0.94→**1.11** 로 개선되었습니다.

### 강점과 약점

| 강할 때 | 약할 때 |
|---------|---------|
| 추세가 뚜렷한 강세장·테마장 | 횡보·박스권 → 잦은 교체로 비용 손실 |
| 위기 진입 시 현금 회피로 방어 | V자 급반등 → 초기 상승 일부 놓침(후행성) |

핵심 약점은 **전환점 지각(lag)** 입니다. 추세가 꺾인 뒤 갈아타므로 천장·바닥을 정확히 잡지 못하는 대신, **큰 추세는 끝까지 타고 큰 폭락은 현금으로 피하는** 거래입니다.

```bash
python cli.py backtest run --strategy topn --rebalance monthly
python -m scripts.topn_sweep     # 위 튜닝 결과 재현
```
